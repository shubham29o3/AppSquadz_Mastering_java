...
class Query01
{
	...
	{
		StringBuffer strQuery=new StringBuffer("select");//to stor prepared query
...
class QueryEvent implements ActionListener
{
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		if(ae.getSource()==cff.jb1)
		{
            ...
            cff.jlist2.setListData(Connect.fieldNames);  //will set colunmn into jlist2
		...
		if(ae.getSource()==cff.jb2)
		{
			...
			int count=0;   //count store no of elements in jlist144
			count=Connect.columnNames.size();
			
			for(int i=0;i<count;i++)//finds which indice are selcted in jlist1
			if(cff.jlist1.isSelectedIndex(i)==true)
				sel[i]=1;
			
			for(int i=0;i<count;i++)  //add selected indices of jlist to jlist2
			if(sel[i]==1)
				...
				//System.out.println("0"+sel[0]+" "+sel[1]+" "+sel[2]+" "+sel[3]+" "+sel[4]+" "+sel[5]+" "+sel[6]+" "+"");
		...
		if(ae.getSource()==cff.jb3)
		{
			...
			int count=0;   //count store no of elements in jlist2
			count=Connect.fieldNames.size();
			System.out.println("##"+count);
			
			for(int i=0;i<count;i++)//finds which indice are selcted in jlist2
			if(cff.jlist2.isSelectedIndex(i)==true)
				sel[i]=1;
			
			for(int i=0;i<count;i++)  //removeselected indices of jlist to jlist2
			if(sel[i]==1)
	...
	if(ae.getSource()==cff.jb6)
	{
		...
		cff.strQuery.append(cff.criteriaBuffer); //now append criteria cluse to query
	...
	{
		cff.jf1=new JFrame("order by option");
		cff.jf1.setSize(400,350);
		cff.jf1.setLocation(150,130);
		cff.jf1.setLayout(null);
        ...
        cff.jf1.add(cff.jcom11);

        cff.jrb11.setBounds(100,150,100,25);
         cff.jf11.add(cff.jrb11);		
		 cff.jrb11.setBounds(220,150,100,25);
...
class TableResult
{
	...
	TableResult(StringBuffer sb)
	{
		...
		{
			System.out.println("tableResult Tble does not exist");
		}
		catch(Exception e)
		{
...
class Connect{
	...
	public void connectDSN()
	{
		try
		{
			dsnName=new String("jdbc:odbc:"+dsnName);
